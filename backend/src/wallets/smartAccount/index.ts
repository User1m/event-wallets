...
program
  ...
  .action(async (opts) => {
    transfer(config, opts.to, opts.amount, Boolean(opts.withPaymaster));
  });
...
program
  ...
  .action(async (opts) => {
    erc20Transfer(config, opts.token, opts.to, opts.amount, Boolean(opts.withPaymaster));
  });
...
program
  ...
  .requiredOption('-amt, --amount <eth>', 'Amount in ETH to transfer')
  .action(async (opts) => {
    batchTransfer(config, opts.to.split(','), opts.amount, Boolean(opts.withPaymaster));
  });
...
program
  ...
  .requiredOption('-amt, --amount <decimal>', 'Amount of the token to transfer')
  .action(async (opts) => {
    batchErc20Transfer(config, opts.token, opts.to.split(','), opts.amount, Boolean(opts.withPaymaster));
  });
